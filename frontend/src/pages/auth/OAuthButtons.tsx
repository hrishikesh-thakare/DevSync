...
/**
 * Kicks off the Supabase-hosted OAuth handshake. Supabase owns the redirect to
 * the provider and back; we only nominate the return address, which is handled
 * by `OAuthCallbackPage` and traded for an app JWT at `POST /auth/oauth/callback`.
 */
...
export function OAuthButtons({ disabled }: { disabled?: boolean }) {
  ...
  const signIn = async (provider: 'github' | 'google') => {
    ...
    try {
      ...
      // On success the browser navigates away, so `pending` is never cleared here.
    } catch (err) {
      setPending(null);
      toast.error(err instanceof Error ? err.message : `Could not start ${provider} sign-in.`);
    }
  };
