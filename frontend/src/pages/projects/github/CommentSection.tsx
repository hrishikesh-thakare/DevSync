  ...
  return (
    <div className="mt-2 bg-background border border-border rounded-lg overflow-hidden flex flex-col max-h-[400px]">
      <div className="flex items-center justify-between px-3 py-2 border-b border-border bg-card">
        ...
        <Button onClick={onClose} className="text-xs text-subtle-foreground hover:text-foreground transition-colors" variant="ghost" size="default">Close</Button>
      </div>
      ...
      <div className="overflow-y-auto flex-1 p-3 space-y-3">
        {isLoading ? (
          ...
          <div className="text-center py-4 text-xs text-danger">{error}</div>
        ) : comments.length === 0 ? (
          <div className="text-center py-4 text-xs text-subtle-foreground">No comments yet. Be the first to comment!</div>
        ) : (
            ...
            <div key={comment.id} className="flex space-x-3">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-baseline space-x-2">
                  <span className="text-xs font-bold text-foreground">{comment.user.login}</span>
                  <span className="text-[10px] text-subtle-foreground">
                    {formatDistanceToNow(new Date(comment.createdAt), { addSuffix: true })}
                  </span>
                </div>
                <div className="mt-0.5 text-xs text-foreground whitespace-pre-wrap break-words bg-card border border-border p-2 rounded">
                  {comment.body}
                </div>
              </div>
            </div>
